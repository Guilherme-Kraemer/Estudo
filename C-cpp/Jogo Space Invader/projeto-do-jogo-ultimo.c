...
#include <stdbool.h>
#include <windows.h>
#include "../../../../.vscode/output/ucrt64/lib/gcc/x86_64-w64-mingw32/12.2.0/include/stdbool.h"
...
void Input()
{
    if (_kbhit())
    {
        ...
        {
        case 'a':
            if (playerX > 1) // Limita o movimento do jogador para n�o sair da borda
                playerX--;
                bufferplayer = playerX+1;
            break;
        case 'd':
            if (playerX < LAR) // Limita o movimento do jogador para n�o sair da borda
                playerX++;
                bufferplayer = playerX-1;
            break;
        case 'q':
...
int main()
{
    ...
    while (!gameover)
    {
        setCursorPosition(0, 0); // Posiciona o cursor no in�cio da tela
        Draw();
        Input();
        Logic();
    }
